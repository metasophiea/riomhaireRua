...
//program manipulation
    void programManager::clearProgram(){ if(debugMode){ std::cout << "clearing program" << std::endl; } program = new std::string[maxProgramLength]; }
    void programManager::load(std::string newProgram[], unsigned int newProgramLength){ if(debugMode){ std::cout << "loading in new program" << std::endl; } 
        ...
        programLength = newProgramLength; if(debugMode){ std::cout << "newProgramLength: " << newProgramLength << std::endl; } 
        for(unsigned int a = 0; a < newProgramLength; a++){ if(debugMode){std::cout << a << "\t" << newProgram[a] << std::endl;} program[a] = newProgram[a]; }
        if(debugMode){ std::cout << "program loaded" << std::endl; } 
    }

    void programManager::appendInstruction(std::string instruction){ if(debugMode){ std::cout << "appending command" << std::endl; } 
        ...
        program[programLength] = instruction; if(debugMode){ std::cout << "new instruction: " << instruction << " at position " << programLength << std::endl; } 
        programLength++; 
    }
    ...
    void programManager::run(){
        ...
        while(true){
            ...
            if(debugMode){ std::cout << "next command number: " << nextCommand << " | " << program[nextCommand] << std::endl; }
...
//printers and debug
    ...
    void programManager::debug(bool onOff){ debugMode = onOff; }